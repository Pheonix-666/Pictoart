...
class TestArtRulesConfig:
    """Tests for the rule-governed art configuration system."""

    def test_load_art_rules_structure(self):
        from src.art_engine.art_config import load_art_rules, ART_RULES
        rules = load_art_rules()
        assert isinstance(rules, dict)
        assert "canvas" in rules
        assert "hatching" in rules
        assert "typography" in rules
        assert "segmentation" in rules
        assert len(rules["hatching"]["angles_deg"]) >= 3
        assert len(rules["typography"]["font_hierarchy"]) >= 3

    def test_art_rules_constants_loaded(self):
        from src.art_engine.art_config import OUTPUT_PORTRAIT_SIZE, FACE_FEATHER_RADIUS
        assert isinstance(OUTPUT_PORTRAIT_SIZE, tuple)
        assert len(OUTPUT_PORTRAIT_SIZE) == 2
        assert FACE_FEATHER_RADIUS > 0
